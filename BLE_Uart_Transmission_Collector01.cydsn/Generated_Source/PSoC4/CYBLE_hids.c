...
CYBLE_GATT_ERR_CODE_T CyBle_HidssWriteEventHandler(CYBLE_GATTS_WRITE_REQ_PARAM_T *eventParam)
{
    ...
    if(NULL != CyBle_HidsApplCallback)
    {
        do
        {
            ...
            if(eventParam->handleValPair.attrHandle == cyBle_hidss[locServiceIndex].protocolModeHandle)
            {
                CyBle_HidssOnProtocolModeWrite(locServiceIndex, eventParam);
            }
            /* Control Point characteristic write request */
            else if(eventParam->handleValPair.attrHandle == cyBle_hidss[locServiceIndex].controlPointHandle)
            {
                CyBle_HidssOnControlPointWrite(locServiceIndex, eventParam);
            }
            else if(eventParam->handleValPair.attrHandle ==
                        cyBle_hidss[locServiceIndex].bootReportArray[CYBLE_HIDS_BOOT_KYBRD_IN_REP_INDX].reportHandle)
            {
                CyBle_HidssReportValueWriteEventHandler(locServiceIndex, CYBLE_HIDS_BOOT_KYBRD_IN_REP, eventParam);
            }
            else if(eventParam->handleValPair.attrHandle ==
                        cyBle_hidss[locServiceIndex].bootReportArray[CYBLE_HIDS_BOOT_KYBRD_IN_REP_INDX].cccdHandle)
            {
                CyBle_HidssCccdWriteEventHandler(locServiceIndex, CYBLE_HIDS_BOOT_KYBRD_IN_REP, eventParam);
            }
            else if(eventParam->handleValPair.attrHandle ==
                        cyBle_hidss[locServiceIndex].bootReportArray[CYBLE_HIDS_BOOT_KYBRD_OUT_REP_INDX].reportHandle)
            {
                CyBle_HidssReportValueWriteEventHandler(locServiceIndex, CYBLE_HIDS_BOOT_KYBRD_OUT_REP, eventParam);
            }
            else if(eventParam->handleValPair.attrHandle ==
                        cyBle_hidss[locServiceIndex].bootReportArray[CYBLE_HIDS_BOOT_MOUSE_IN_REP_INDX].reportHandle)
            {
                CyBle_HidssReportValueWriteEventHandler(locServiceIndex, CYBLE_HIDS_BOOT_MOUSE_IN_REP, eventParam);
            }
            else if(eventParam->handleValPair.attrHandle ==
                        cyBle_hidss[locServiceIndex].bootReportArray[CYBLE_HIDS_BOOT_MOUSE_IN_REP_INDX].cccdHandle)
            {
                CyBle_HidssCccdWriteEventHandler(locServiceIndex, CYBLE_HIDS_BOOT_MOUSE_IN_REP, eventParam);
            }
            else
            {
                locCharIndex = CYBLE_HIDS_REPORT;
                for(reportIndex = 0u; reportIndex < cyBle_hidss[locServiceIndex].reportCount; reportIndex++)
                {
                    if(eventParam->handleValPair.attrHandle == 
                        cyBle_hidss[locServiceIndex].reportArray[reportIndex].reportHandle)
                    {
                        
                        CyBle_HidssReportValueWriteEventHandler(locServiceIndex, locCharIndex, eventParam);
                    }
                    if(eventParam->handleValPair.attrHandle == 
                        cyBle_hidss[locServiceIndex].reportArray[reportIndex].cccdHandle)
                    {
                        CyBle_HidssCccdWriteEventHandler(locServiceIndex, locCharIndex, eventParam);
                    }
                    locCharIndex++;
                }
            }
        ...
        }while(locServiceIndex < CYBLE_HIDSS_SERVICE_COUNT);
...
void CyBle_HidssOnDeviceConnected(void)
{
    ...
    do
    {
        if(cyBle_hidss[locServiceIndex].protocolModeHandle !=
           CYBLE_GATT_INVALID_ATTR_HANDLE_VALUE)
        {
            /* Store default protocol value in database */
            CYBLE_GATT_DB_ATTR_SET_GEN_VALUE(cyBle_hidss[locServiceIndex].protocolModeHandle,
...
void CyBle_HidscNotificationEventHandler(CYBLE_GATTC_HANDLE_VALUE_NTF_PARAM_T *eventParam)
{
    ...
    if(NULL != CyBle_HidsApplCallback)
    {
        for(locServiceIndex = 0u; locServiceIndex < CYBLE_HIDSC_SERVICE_COUNT; locServiceIndex++)
        {
            for(locCharIndex = CYBLE_HIDS_PROTOCOL_MODE; locCharIndex < CYBLE_HIDS_CHAR_COUNT; locCharIndex++)
            {
                if(CyBle_HidscGetCharacteristicValueHandle(locServiceIndex, locCharIndex) == 
                                                                            eventParam->handleValPair.attrHandle)
                {
                    notifValue.connHandle = eventParam->connHandle;
                    notifValue.serviceIndex = locServiceIndex;
...
void CyBle_HidscReadResponseEventHandler(CYBLE_GATTC_READ_RSP_PARAM_T *eventParam)
{
    ...
    {
        for(locServiceIndex = 0u; (locServiceIndex < CYBLE_HIDSC_SERVICE_COUNT) && (locReqHandle == 0u); locServiceIndex++)
        {
            for(locCharIndex = CYBLE_HIDS_PROTOCOL_MODE; (locCharIndex < CYBLE_HIDS_CHAR_COUNT) && (locReqHandle == 0u); 
                locCharIndex++)
            {
                if(cyBle_hidscReqHandle == CyBle_HidscGetCharacteristicValueHandle(locServiceIndex, locCharIndex))
                {
                    CYBLE_HIDS_CHAR_VALUE_T locCharValue;
                    
                    locCharValue.connHandle = eventParam->connHandle;
                    locCharValue.serviceIndex = locServiceIndex;
                ...
                {
                    CYBLE_HIDS_DESCR_T locDescrIndex;

                    for(locDescrIndex = CYBLE_HIDS_REPORT_CCCD; (locDescrIndex < CYBLE_HIDS_DESCR_COUNT) &&
                       (locReqHandle == 0u); locDescrIndex++)
                    {
                        if(cyBle_hidscReqHandle == CyBle_HidscGetCharacteristicDescriptorHandle(
                                                        locServiceIndex, locCharIndex, locDescrIndex))
                        {
                            CYBLE_HIDS_DESCR_VALUE_T locDescrValue;
                            
                            locDescrValue.connHandle = eventParam->connHandle;
                            locDescrValue.serviceIndex = locServiceIndex;
