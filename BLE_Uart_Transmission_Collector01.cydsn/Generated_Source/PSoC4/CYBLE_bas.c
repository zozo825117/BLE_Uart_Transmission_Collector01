...
void CyBle_BasInit(void)
{
    ...
    (void)memset(&cyBle_basc, 0, sizeof(cyBle_basc));
...
CYBLE_GATT_ERR_CODE_T CyBle_BassWriteEventHandler(CYBLE_GATTS_WRITE_REQ_PARAM_T *eventParam)
{
    ...
    if(NULL != CyBle_BasApplCallback)
    {
        do
        {
            ...
            if(eventParam->handleValPair.attrHandle == cyBle_bass[locServiceIndex].cccdHandle)
            {
                /* Verify that optional notification property is enabled for Battery Level characteristic */
                if(CYBLE_IS_NOTIFICATION_SUPPORTED(cyBle_bass[locServiceIndex].batteryLevelHandle))
                {
                    ...
                    if(CYBLE_GATT_ERR_NONE == gattErr)
                    {
                        ...
                        locCharIndex.serviceIndex = locServiceIndex;
        ...
        locServiceIndex++;
        }while(locServiceIndex < CYBLE_BASS_SERVICE_COUNT);
    }
    return (gattErr);
}
...
Parameters:
 connHandle: The BLE peer device connection handle.
 serviceIndex: Index of the service instance. e.g. If two Battery Services 
                are supported in your design, then first service will be 
                identified by serviceIndex of 0 and the second by
                serviceIndex of 1.
 charIndex: The index of a service characteristic of type 
...
void CyBle_BascReadResponseEventHandler(CYBLE_GATTC_READ_RSP_PARAM_T *eventParam)
{
    ...
    {
        for(locServiceIndex = 0u; (locServiceIndex < CYBLE_BASC_SERVICE_COUNT) && (locReqHandle == 0u); locServiceIndex++)
        {
            if(cyBle_basc[locServiceIndex].batteryLevel.valueHandle == cyBle_bascReqHandle)
            {
                CYBLE_BAS_CHAR_VALUE_T batteryLevelValue;
                
                batteryLevelValue.connHandle = eventParam->connHandle;
                batteryLevelValue.serviceIndex = locServiceIndex;
            ...
            else if( (cyBle_basc[locServiceIndex].cccdHandle == cyBle_bascReqHandle) ||
                     (cyBle_basc[locServiceIndex].cpfdHandle == cyBle_bascReqHandle) )
            {
                ...
                locDescrValue.descrIndex = ((cyBle_basc[locServiceIndex].cccdHandle == cyBle_bascReqHandle) ? 
...
void CyBle_BascWriteResponseEventHandler(const CYBLE_CONN_HANDLE_T *eventParam)
{
    ...
    if((NULL != CyBle_BasApplCallback) && (CYBLE_GATT_INVALID_ATTR_HANDLE_VALUE != cyBle_bascReqHandle))
    {
        for(locServiceIndex = 0u; locServiceIndex < CYBLE_BASC_SERVICE_COUNT; locServiceIndex++)
        {
            if(cyBle_basc[locServiceIndex].cccdHandle == cyBle_bascReqHandle)
            {
                CYBLE_BAS_DESCR_VALUE_T locDescrIndex;
                
                locDescrIndex.connHandle = *eventParam;
                locDescrIndex.serviceIndex = locServiceIndex;
                locDescrIndex.charIndex = CYBLE_BAS_BATTERY_LEVEL;
                locDescrIndex.descrIndex = CYBLE_BAS_BATTERY_LEVEL_CCCD;
                locDescrIndex.value = NULL;
                
                cyBle_bascReqHandle = CYBLE_GATT_INVALID_ATTR_HANDLE_VALUE;
                cyBle_eventHandlerFlag &= (uint8)~CYBLE_CALLBACK;
                CyBle_BasApplCallback((uint32)CYBLE_EVT_BASC_WRITE_DESCR_RESPONSE, &locDescrIndex);
                break;
            }
        }
    }
...
{
    if((NULL != eventParam) && (CYBLE_GATT_INVALID_ATTR_HANDLE_VALUE != cyBle_bascReqHandle))
    {
        cyBle_bascReqHandle = CYBLE_GATT_INVALID_ATTR_HANDLE_VALUE;
    }
}
