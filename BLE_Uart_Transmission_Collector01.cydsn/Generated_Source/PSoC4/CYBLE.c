...
CYBLE_GAP_BD_ADDR_T cyBle_deviceAddress = {{0x07u, 0x00u, 0x00u, 0x50u, 0xA0u, 0x00u}, 0x00u };
...
Return:
 ...
 Global variables:
  The CYBLE_intiVar variable is used to indicate initial
...
CYBLE_API_RESULT_T CyBle_Start(CYBLE_CALLBACK_T callbackFunc)
{
    ...
    if(callbackFunc != NULL)
    {
        ...
        apiResult = CyBle_StackInit(&CyBle_EventHandler, cyBle_stackMemoryRam, CYBLE_STACK_RAM_SIZE, 
            dataBuff, CYBLE_STACK_BUF_COUNT, cyBle_StackFlashptr,CYBLE_STACK_FLASH_SIZE);
    #else
        apiResult = CyBle_StackInit(&CyBle_EventHandler, cyBle_stackMemoryRam, CYBLE_STACK_RAM_SIZE,
            NULL, 0u, NULL, 0u);
    #endif /* CYBLE_MODE_PROFILE */  
...
CYBLE_API_RESULT_T CyBle_StoreBondingData(uint8 isForceWrite)
{
    ...
    *******************************************************************************
    ...
    Parameters:
     ...
     uint8 dest: 0 - selects advertisement structure, not zero value selects scan 
                 response structure.
